...
def parse_kannada_transaction(
    ...
    try:
        with open(log_path, 'a', encoding='utf-8') as f:
            f.write(json.dumps({'location':'voice_parser.py:477','message':'Kannada customer extraction start','data':{'transcript':transcript[:100],'customer_list_len':len(customer_list) if customer_list else 0},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H1'})+'\n')
    except: pass
    # #endregion
    ...
    for pattern in customer_patterns:
        ...
        if match:
            ...
            try:
                with open(log_path, 'a', encoding='utf-8') as f:
                    f.write(json.dumps({'location':'voice_parser.py:495','message':'Pattern matched','data':{'pattern':pattern,'matched_text':potential_name},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H1'})+'\n')
            except: pass
            # #endregion
            ...
            for postpos in kannada_postpositions:
                if potential_name.endswith(postpos):
                    ...
                    try:
                        with open(log_path, 'a', encoding='utf-8') as f:
                            f.write(json.dumps({'location':'voice_parser.py:502','message':'Postposition stripped','data':{'original':original_name,'stripped':potential_name,'postpos':postpos},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H3'})+'\n')
                    except: pass
                    # #endregion
                    break
            ...
            try:
                with open(log_path, 'a', encoding='utf-8') as f:
                    f.write(json.dumps({'location':'voice_parser.py:506','message':'Fuzzy match result','data':{'potential_name':potential_name,'customer_id':customer_id},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H2'})+'\n')
            except: pass
            # #endregion
            if customer_id:
                for customer in customer_list:
            ...
            if match:
                ...
                if potential_name and len(potential_name) > 1:
                    ...
                    if not potential_name.isdigit() and len(potential_name) >= 2:
                        ...
                        try:
                            with open(log_path, 'a', encoding='utf-8') as f:
                                f.write(json.dumps({'location':'voice_parser.py:560','message':'Using extracted name as fallback','data':{'customer_name':customer_name},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H2'})+'\n')
                        except: pass
                        # #endregion
                        break
    ...
    if customer_name is None:
        ...
        try:
            with open(log_path, 'a', encoding='utf-8') as f:
                f.write(json.dumps({'location':'voice_parser.py:601','message':'Entering first word fallback','data':{'transcript':transcript},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H1'})+'\n')
        except: pass
        # #endregion
        ...
        for word in words:
            ...
            for postpos in kannada_postpositions:
                if word.endswith(postpos):
                    ...
                    try:
                        with open(log_path, 'a', encoding='utf-8') as f:
                            f.write(json.dumps({'location':'voice_parser.py:615','message':'Stripping postposition in fallback','data':{'word':word,'base_word':base_word,'postpos':postpos},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H3'})+'\n')
                    except: pass
                    # #endregion
                    break
            if base_word and len(base_word) >= 2:
                ...
                try:
                    with open(log_path, 'a', encoding='utf-8') as f:
                        f.write(json.dumps({'location':'voice_parser.py:622','message':'Using first word fallback','data':{'customer_name':customer_name,'word':word,'base_word':base_word},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H1'})+'\n')
                except: pass
                # #endregion
                break
    ...
    try:
        with open(log_path, 'a', encoding='utf-8') as f:
            f.write(json.dumps({'location':'voice_parser.py:662','message':'Parser result assembled','data':{'customer_id':customer_id,'customer_name':customer_name,'type':tx_type,'amount':amount},'timestamp':int(__import__('time').time()*1000),'sessionId':'debug-session','runId':'run1','hypothesisId':'H4'})+'\n')
    except: pass
    # #endregion
